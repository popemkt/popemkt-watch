...
- One channel `reminders_v4`, importance HIGH (required for full-screen intent launch), **default notification sound** + an aggressive multi-buzz vibration pattern (~5 s salvo; `FLAG_INSISTENT` does not loop vibration on the test device, so the pattern itself must carry the urgency). Superseded channels (`reminders`, `reminders_alarm`) are deleted on startup (channels are immutable — sound changes require a new channel id). `TODO NGH:` canonical expectation: the bundled loud tone on the alarm stream (`watchcal_alarm.wav`, kept in `res/raw`); current implementation: default notification sound, because on the Xiaomi Watch 5 a channel configured with a custom `android.resource://` sound + `USAGE_ALARM` was *fully suppressed* — no sound, no vibration (specs/learnings.md); impact: ring loudness rides the notification stream and the OEM default tone; closes: bisect on-device (custom tone on notification stream vs default tone on alarm stream) and restore whichever half survives.
...
- **Agenda is a Wear `Scaffold` (TimeText + Vignette + PositionIndicator) over a `ScalingLazyColumn`.** The list interleaves `ListHeader` day-group rows (`Today`/`Tomorrow`/weekday, bucketed by local-midnight from `beginMillis`) with event chips, ends in a `CompactChip` gear to Settings, and surfaces the full-screen-intent grant only as a single warning chip when the appop is missing. State is conveyed by a leading text glyph (`○`/`Zz`/`✓`) plus muted chip colors + strikethrough for Done — no icon-font dependency is added (text glyphs match the existing `Zz`/`✓` buttons and honor the minimize-complexity rule). The secondary label names the next tap's outcome, keeping the one-gesture cycle self-documenting. Grouping is a pure UI concern computed in `AgendaScreen`; the `AgendaEntry` model is unchanged.
